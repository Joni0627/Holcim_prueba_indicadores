...
/**
 * Fetch rows filtered by a date range directly in Supabase (server-side filtering).
 * Far more efficient than fetchAllRows + JS filter for large tables.
 * @param tableName  - Supabase table name
 * @param dateColumn - Exact DB column name to filter on (e.g. "fecha")
 * @param startDate  - ISO date string YYYY-MM-DD (inclusive)
 * @param endDate    - ISO date string YYYY-MM-DD (inclusive)
 * @param columns    - Comma-separated column list for SELECT (default: '*')
 */
export async function fetchRowsByDateRange(
    tableName: string,
    dateColumn: string,
    startDate: string,
    endDate: string,
    ...
    const candidateColumns = Array.from(new Set([dateColumn, dateColumn.toLowerCase(), dateColumn.toUpperCase(), "fecha", "FECHA", "date"]));
    ...
    for (const col of candidateColumns) {
        ...
        try {
            ...
            if (!queryFailed) {
                console.log(`[Supabase] Date-filtered fetch complete using column '${col}': ${allRows.length} rows from ${tableName}`);
                return allRows;
            }
        } catch (err: any) {
            console.warn(`[Supabase] Exception testing date column '${col}' on ${tableName}:`, err);
        }
    }

    console.warn(`[Supabase] All candidate date columns failed for ${tableName}. Falling back to full fetchAllRows`);
    return fetchAllRows(tableName);
}

/**
 * Fetch child-table rows by a list of parent IDs (e.g. detalles_produccionv2 by produccion_id).
 * Chunks requests into groups of 500 to avoid PostgREST URL length limits.
 * @param tableName - Supabase table name
 * @param idColumn  - FK column to filter on (e.g. "produccion_id")
 * @param ids       - Array of ID values to match
 * @param columns   - Comma-separated column list for SELECT (default: '*')
 */
export async function fetchRowsByIds(
    ...
    console.log(`[Supabase] ID-filtered fetch: ${tableName} WHERE ${idColumn} IN [${ids.length} ids]`);
    ...
    for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
        ...
        if (error) {
            console.error(`[Supabase] Error on ID-filtered fetch of ${tableName} (chunk ${i / CHUNK_SIZE}):`, error);
            continue;
        }
        if (data) allRows.push(...data);
    }

    console.log(`[Supabase] ID-filtered fetch complete: ${allRows.length} rows from ${tableName}`);
    return allRows;
}
